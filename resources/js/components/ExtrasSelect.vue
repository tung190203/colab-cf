...
/**
 * Hàm tính giá hiển thị cho 1 item dựa trên số lượng free
 * Free theo thứ tự chọn trước
 */
 function getItemDisplayPrice(item, category) {
  const selectedItem = form[category].find((e) => e.id === item.id);
  if (!selectedItem) {
    return item.price;
  }
  return selectedItem.totalPrice || 0;
}
...
<template>
  <div>
        ...
        >
          ...
          <div class="d-flex align-items-center gap-2">
            <div class="text-warning fw-bold">
              {{ formatVND(getItemDisplayPrice(item, category)) }}
            </div>
            <input
    ...
    <div class="d-flex justify-content-between gap-3">
      <button class="btn btn-outline-secondary flex-grow-1" @click="goBack">
        Quay lại
      </button>
      <button class="btn btn-warning flex-grow-1" @click="goNext">
        Tiếp tục
      </button>
    </div>
  </div>

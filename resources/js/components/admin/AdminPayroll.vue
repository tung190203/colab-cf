...
function exportPayroll() {
    ...
    exportRowsToExcel({
        ...
        columns: [
            ...
            { header: 'Lương/giờ', value: item => Number(item.payroll?.hourly_rate ?? item.hourly_rate ?? 0), numberFormat: '#,##0 "₫"' },
            { header: 'Lương tính', value: item => payrollBaseSalary(item), numberFormat: '#,##0 "₫"' },
            { header: 'Phụ cấp', value: item => Number(item.payroll ? item.payroll.bonus : item.bonus ?? 0), numberFormat: '#,##0 "₫"' },
            { header: 'Chi tiết phụ cấp', value: item => detailText(item.payroll?.bonus_details ?? item.bonus_details) },
            { header: 'Phạt', value: item => Number(item.payroll?.deduction ?? 0), numberFormat: '#,##0 "₫"' },
            { header: 'Chi tiết phạt', value: item => detailText(item.payroll?.deduction_details) },
            { header: 'Thực nhận', value: item => computedTotal(item), numberFormat: '#,##0 "₫"' },
